...
| `memory` | bool | Whether this persona reads and writes [memory](Memory) at all. Absent, or anything other than a plain boolean, means off. Off means no recall, no memory tools offered, no review pass, and the persona's memory store is left untouched. **The edit screen's *Memory* box starts unticked for a brand-new persona and an existing persona shows its own file's value** — ticking it writes `memory = true` explicitly; unticking it removes the key. |
| `workspace` | bool | Gives this persona a [workspace](Workspace): a folder per conversation, where a tool's fetched files and the persona's own writes are kept. **Off by default** — the opposite of `memory` above — so absent means off. Set from the edit screen's *Workspace* checkbox. |
| `workspace_pins` | array of strings | Filename patterns (globs — `B*_Ch*.md`) whose matching workspace files are shown to the persona whole, every turn, rather than only listed. Absent or empty means none pinned. Set from the edit screen's *Pinned files* box, comma separated there and a list here. |
| `thinking` | bool | Whether this persona's replies reason before they answer. Absent, or anything other than a plain boolean, means on. Off is faster and cheaper; on is better for review and editing. Set from the edit screen's *Thinking* checkbox, directly below Workspace; a conversation's own header checkbox can override it for that thread. How much reasoning a reply may spend is set on the model server, not per persona. |
