...
| `workspace` | bool | Gives this persona a [workspace](Workspace): a folder per conversation, where a tool's fetched files and the persona's own writes are kept. **Off by default** — the opposite of `memory` above — so absent means off. Set from the edit screen's *Workspace* checkbox. There is no persona-level pinning — [pinning a file](Workspace) is a per-conversation action, not a persona setting. |
